...
class StopWatchTimer {
  ...
  /// Get millie second from hour
  static int getMillieSecFromHour(int hour) => (hour * (3600 * 1000)).floor();

  /// Get millie second from minute
  static int getMillieSecFromMinute(int minute) => (minute * 60000).floor();

  /// Get millie second from second
  static int getMillieSecFromSecond(int second) => (second * 1000).floor();
  ...
  void clearPresetTime() {
    if (mode == StopWatchMode.countUp) {
      ...
      _elapsedTime
          .add(isRunning ? _getCountUpTime(_presetTime!) : _presetTime!);
  ...
  void _lap() {
    if (isRunning) {
      final rawValue = _rawTimeController.valueWrapper?.value ?? 0;
      _records!.add(
        StopWatchRecord(
          rawValue: rawValue,
